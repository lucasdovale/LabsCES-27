...
func newMaster(address string) (master *Master) {
	...
	master.failedWorkerChan = make(chan *RemoteWorker, FAILED_WORKER_BUFFER)
...
func (master *Master) handleFailingWorkers() {
	...
	// descobrindo o worker falho iterando no canal
	for workerFailed := range master.failedWorkerChan {
		// retirar o worker da lista de workers disponíveis
		master.workersMutex.Lock()
		log.Printf("Removing worker %d from master list.\n", workerFailed.id)
		delete(master.workers, workerFailed.id)
		workerFailed.status = WORKER_UNAVAILABLE
