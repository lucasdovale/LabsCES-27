...
func (master *Master) schedule(task *Task, proc string, filePathChan chan string) int {
	...
	for filePath = range filePathChan {
		select {
		case opFailed := <-master.failedOperationsChan:
			log.Printf("Retrying to run failed operation %d\n", opFailed.id)
			worker = <-master.idleWorkerChan
			wg.Add(1)
			go master.runOperation(worker, opFailed, &wg)
		default:
			// log.Printf("No operations failed")
		}
